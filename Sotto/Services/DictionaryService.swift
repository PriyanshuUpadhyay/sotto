...
enum DictionaryService {
    ...
    /// Returns an error message string if something went wrong, nil on success.
    ///
    /// `sortOrder` (P3.D) controls placement in manual sort. Default `nil`
    /// preserves legacy callers (`DictionaryQuickAddPanel`, import flow) —
    /// they land at 0 alongside other unordered rows. Pass `currentMax + 1`
    /// to push new entries to the bottom of an existing manual order.
    @discardableResult
    static func addWordReplacement(
        ...
        if let sortOrder {
            ...
            entry = WordReplacement(originalText: original, replacementText: replacement)
        }
        context.insert(entry)
        do {
            try context.save()
