...
final class SessionMetricMigrationService {
    ...
    private let logger = Logger(subsystem: OSLogSubsystems.app, category: "SessionMetricMigrationService")
    private let completionKey = "HasCompletedStatsMigration"
    private(set) var isRunning = false

    private init() {}

    /// Cherry-pick of upstream `9864d6d` + race-fix layer from `a374702`.
    /// Fork adaptation: upstream packs `Transcription` and `SessionMetric` into
    /// the same `ModelContainer`. Our fork keeps stats in a separate local-only
    /// `stats.store` container (f02), so the migration takes BOTH containers
    /// and fans the reads/writes across them. The single-ID-set fetch
    /// optimization is preserved (one `FetchDescriptor<SessionMetric>` on the
    /// stats context to build the skip-set, then one `FetchDescriptor<Transcription>`
    /// on the transcript context).
    @discardableResult
    func runIfNeeded(transcriptContainer: ModelContainer, statsContainer: ModelContainer) -> Task<Void, Never>? {
        ...
        return Task.detached(priority: .utility) {
            let transcriptContext = ModelContext(transcriptContainer)
            ...
            do {
                ...
                let existingIds = Set(
                    try statsContext.fetch(FetchDescriptor<SessionMetric>())
                ...
                let transcriptions = try transcriptContext.fetch(descriptor)
