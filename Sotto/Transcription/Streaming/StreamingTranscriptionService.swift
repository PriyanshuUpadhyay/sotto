...
class StreamingTranscriptionService {
    ...
    private let logger = Logger(subsystem: OSLogSubsystems.app, category: "StreamingTranscriptionService")
    ...
    init(
        ...
        streamingManagerCache: FluidAudioStreamingManagerCache? = nil,
        onPartialTranscript: ((String) -> Void)? = nil
    ) {
        self.modelContext = modelContext
        self.fluidAudioService = fluidAudioService
        self.streamingManagerCache = streamingManagerCache
    ...
    func startStreaming(model: any TranscriptionModel) async throws {
        ...
        // `transcribe`, so clear any prior file-decode boosting outcome to keep it
        // off this utterance's trace. No-op for non-FluidAudio providers.
        await fluidAudioService?.resetBoostingTrace()
    ...
    private func recordObservedTranscript(_ text: String) {
        let count = text.unicodeScalars.filter { !$0.properties.isWhitespace }.count
        if count > maxObservedTranscriptLength {
            maxObservedTranscriptLength = count
        }
    }

    func createProvider(for model: any TranscriptionModel) -> StreamingTranscriptionProvider {
        if model.provider == .fluidAudio {
            if FluidAudioModelManager.isStreamingManagerCacheFamily(named: model.name) {
                guard let streamingManagerCache else {
                    fatalError("FluidAudioStreamingManagerCache required for FluidAudio streaming. Ensure it is passed to StreamingTranscriptionService.")
                }
                if FluidAudioModelManager.isParakeetUnifiedModel(named: model.name) {
                    return FluidAudioUnifiedStreamingProvider(cache: streamingManagerCache)
                }
                if FluidAudioModelManager.isParakeetEouModel(named: model.name) {
                    return FluidAudioEouStreamingProvider(cache: streamingManagerCache)
                }
                return FluidAudioNemotronStreamingProvider(cache: streamingManagerCache)
            }
            ...
            for await event in events {
                ...
                case .committed(let text):
                    ...
                    await MainActor.run {
                        ...
                        let display = self.committedSegments.joined(separator: " ")
                        // Recorded unconditionally (not just while .streaming) so the final
                        // commit-during-.committing counts as evidence too.
                        self.recordObservedTranscript(display)
                ...
                case .partial(let text):
                    await MainActor.run {
                        // Recorded during both .streaming and .committing: providers can still
                        // emit partials while queued audio drains after stop() flips the state,
                        // and those can be the longest partial seen this session. The UI callback
                        // stays .streaming-only — no live preview once teardown has begun.
                        guard self.state == .streaming || self.state == .committing else { return }
