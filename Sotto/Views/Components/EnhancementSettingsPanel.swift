...
struct EnhancementSettingsPanel: View {
    ...
    @AppStorage("ReviewBeforePaste") private var reviewBeforePaste = true
    ...
    private var cleanupLevelSection: some View {
        sectionBlock(
            ...
            info: "When on, transcripts are cleaned on-device (Apple Foundation Models): fillers removed and punctuation added, wording preserved. When off, the raw transcript is pasted unchanged."
        ) {
            Toggle(isOn: Binding(
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                Toggle(isOn: $reviewBeforePaste) {
                    HStack(spacing: 4) {
                        ...
                        InfoTip("Show the transcript (or enhanced text) in an editable preview before it's pasted. Edit if you want, then press ⌘↵ to paste or Esc to cancel. Any edit you make is captured exactly to improve future enhancement. When off, text auto-pastes and the feedback tray below appears instead.")
                    }
                }
                .toggleStyle(.switch)

                Toggle(isOn: $enhancementService.showFeedbackTray) {
                    HStack(spacing: 4) {
                        Text("Show feedback tray after enhanced paste")
                        InfoTip("After an AI-enhanced paste, show quick controls to mark the raw text better, flag a bad enhancement, or correct it — captured to improve future enhancement.")
                    }
                }
                .toggleStyle(.switch)
                .disabled(reviewBeforePaste)
            }
        }
    }
    ...
    private var onDeviceSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            sectionBlock(label: "ON-DEVICE") {
                VStack(alignment: .leading, spacing: 12) {
                    // Surface the active local path (Apple Foundation Models).
                    ...
                    // AFM prewarm gate. Visible whenever AFM is available so the
                    // user can disable warm() calls and A/B cold-vs-warm ttft.
                    if afmAvailable {
                    ...
                    // enhancement appends one row to the CSV under Application
                    // Support so the user can verify perf changes quantitatively.
                    HStack(spacing: 8) {
                    ...
                    Button {
                        ...
                        Label(isRunningTranscriptionEval ? "Sweeping…" : "Sweep transcription models", systemImage: "waveform.badge.magnifyingglass")
                    }
                    .buttonStyle(.bordered)
                    .disabled(isRunningTranscriptionEval)
                    .help("Re-transcribe last 20 saved recordings through every downloaded ASR model. Writes transcription-eval-<stamp>.md.")
                }
            }
...
//
// Compact section block + footer for narrow popover surfaces. Uses a compact
// section-label vocabulary so the popover reads as ONE glass surface with
// hierarchical sections (no SettingsCard
    ...
    VStack(alignment: .leading, spacing: 10) {
        HStack(spacing: 4) {
            Text(label)
                ...
                .tracking(0.06 * 10.5)
                .foregroundColor(Theme.inkSecondary.opacity(0.7))
            if let info {
                InfoTip(info)
            }
            Spacer()
