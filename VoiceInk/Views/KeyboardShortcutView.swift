        ...
        if let shortcut = shortcut {
            HStack(spacing: 6) {
                ForEach(shortcutComponents(from: shortcut), id: \.self) { component in
                    ShortcutKeyCap(text: component)
                }
            }
        } else {
            ShortcutKeyCap(text: "Not Set")
...
// Legacy key cap, scoped private to this file to avoid colliding with the
// Phase 2 `VoiceInk/Views/Common/KeyCapView.swift` glass cap. Used only by
// `KeyboardShortcutView` above (3 references inside this file). Migrating
// `KeyboardShortcutView` callers to the new `KeyCombo` is out of scope for
// P2.E — see plan reviewer note on `844ebae`.
