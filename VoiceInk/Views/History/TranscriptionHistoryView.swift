...
struct TranscriptionHistoryView: View {
    ...
    /// Watch the shared detector so the sidebar + center pane glass backgrounds
    /// rebuild when the wallpaper flips light↔dark. Established pattern: see
    /// `MenuBarView`, `PromptChipPicker`, `ConstellationContainer`.
    @ObservedObject private var glassAppearance = GlassAppearanceDetector.shared
    ...
    private var leftSidebarView: some View {
        ...
        .background(
            // Adaptive glass: HaloMaterial picks light/onyx via the shared
            // detector so the sidebar's row alternation reads against the
            // wallpaper-driven luminance, not a hardcoded controlBackground.
            HaloMaterial(
                shape: Rectangle(),
                phase: .hidden,
        ...
        Group {
            if let transcription = selectedTranscription {
                TranscriptionDetailView(
                    transcription: transcription,
                    onInfoTap: { withAnimation { isRightSidebarVisible.toggle() } },
                    onDelete: {
    ...
    /// Delete a single transcription record (invoked from `TranscriptionDetailView`'s
    /// action row). Mirrors `deleteSelectedTranscriptions` for the multi-select
    /// path: same `performDeletion` + reload sequence so the list stays consistent.
