...
**Supersedes:** v1 of this spec (drafted before reading the codebase; see git history). Synthesised from `-v2a-minimal.md` (surgical patches) + `-v2b-structural.md` (code-grounded rebuild) following a 3-critic adversarial review.
...
The principle: the app respects the notch as hardware; it summons a feature *on top* of the notch when invoked, and recedes when done. The magic is in the summoning, not the squatting.
