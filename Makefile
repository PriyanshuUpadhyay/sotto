...
# "voiceink-fork-local" in the login keychain so each rebuild keeps a stable
...
#     Name: voiceink-fork-local
#     Identity Type: Self Signed Root
#     Certificate Type: Code Signing
#
# Override with: make local LOCAL_SIGN_IDENTITY="other-name" — exact name or SHA1.
LOCAL_SIGN_IDENTITY ?= $(shell /usr/bin/security find-identity -p codesigning -v 2>/dev/null | grep -F '"voiceink-fork-local"' | head -1 | awk '{ print $$2 }' | grep -E '^[A-F0-9]+$$' || echo "-")

.PHONY: all clean whisper setup build local check healthcheck help dev run reload
